...
def parse_structured_xml(file_path):
    ...
    def process_element(
        element, current_cycle_name=None, cycle_has_title=None, cycle_dedication=None
    ):
        ...
        if element.tag == "SECTION":
            ...
            for child in element:
                process_element(
                    ...
                    cycle_has_title=cycle_has_title,
                    cycle_dedication=cycle_dedication,
                )

        elif element.tag == "CYCLE":
            title_elem = element.find("TITLE")
            notitle_elem = element.find("NOTITLE")

            # Определяем тип цикла
            if title_elem is not None and title_elem.text and title_elem.text.strip():
                cycle_name = title_elem.text.strip()
                cycle_has_title = True
            elif notitle_elem is not None:
                # Используем текст из NOTITLE или генерируем имя
                cycle_name = (
                    notitle_elem.text.strip() if notitle_elem.text else "Без названия"
                )
                cycle_has_title = False
            else:
                cycle_name = None
                cycle_has_title = None
            ...
            poem_data = {
                ...
                "cycle_display_name": current_cycle_name,
                "cycle_has_title": cycle_has_title,
                "cycle_dedication": cycle_dedication,
